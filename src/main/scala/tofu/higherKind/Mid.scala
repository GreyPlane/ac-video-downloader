...
object Mid extends MidInstances {
  ...
  def attach[U[f[_]]: ApplyK, F[_]](up: U[Mid[F, _]])(alg: U[F]): U[F] =
    up.attach(alg)

  implicit final class TofuMidAlgebraSyntax[F[_], U[f[_]]](
      private val self: U[Mid[F, _]]
  ) extends AnyVal {
    def attach(alg: U[F])(implicit U: ApplyK[U]): U[F] =
      U.map2K(alg, self)(
        funK[Tuple2K[F, Mid[F, *], *], F](t2k => t2k.second(t2k.first))
      )
  }

  private final case class MidCompose[F[_], A](elems: Chain[Mid[F, A]])
      extends Mid[F, A] {
...
trait MidInstances extends MidInstances1 {
  ...
  implicit def midAlgebraMonoid[F[_], U[f[_]]: MonoidalK]
      : Monoid[U[Mid[F, _]]] = new MidAlgebraMonoid[F, U]
}

trait MidInstances1 {
  implicit def midAlgebraSemigroup[F[_], U[f[_]]: ApplyK]
      : Semigroup[U[Mid[F, _]]] = new MidAlgebraSemigroup[F, U]

  private val midInvariantInstance: InvariantK[Mid[*[_], Any]] =
    new MidInvariantK()
  implicit def midInvariantK[A]: InvariantK[Mid[*[_], A]] =
    midInvariantInstance
      .asInstanceOf[InvariantK[Mid[*[_], A]]]
}

class MidMonoidK[F[*]] extends MonoidK[Mid[F, *]] {
...
class MidInvariantK extends InvariantK[Mid[*[_], Any]] {
  def imapK[F[_], G[_]](
      af: Mid[F, Any]
  )(fk: F ~> G)(gk: G ~> F): Mid[G, Any] = { ga =>
    fk(af(gk(ga)))
  }
}

class MidAlgebraMonoid[F[_], U[f[_]]: MonoidalK]
    extends MidAlgebraSemigroup[F, U]
    with Monoid[U[Mid[F, _]]] {
  def empty: U[Mid[F, _]] = Mid.point[F].pureK[U]
}

class MidAlgebraSemigroup[F[_], U[f[_]]](implicit U: ApplyK[U])
    extends Semigroup[U[Mid[F, _]]] {
  def combine(x: U[Mid[F, _]], y: U[Mid[F, _]]): U[Mid[F, _]] =
    U.map2K(x, y)(
      funK[Tuple2K[Mid[F, _], Mid[F, _], _], Mid[F, _]](t2 =>
        fa => t2.first(t2.second(fa))
      )
    )
}
