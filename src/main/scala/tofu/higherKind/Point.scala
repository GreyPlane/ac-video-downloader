...
/** equivalent of UnitK ~> F get F[A] for any given type A useful for empty values or typeconstructors with phantom
  * parameters may look like a type class but is not
  */
trait Point[F[_]] {
  def point[A]: F[A]
...
object Point {
  ...
  implicit val pointRepresentable: RepresentableK[Point] = new RepresentableK[Point] {
    def tabulate[F[_]](hom: RepK[Point, _] ~> F): Point[F] = new Point[F] {
