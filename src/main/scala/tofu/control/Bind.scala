...
trait BiMonad[L[_, _], R[_, _]] {
  ...
  def leftFlatMap[A, B, C, D](
      lab: L[A, B]
  )(fl: A => L[C, D], fr: B => R[C, D]): L[C, D]
  def rightFlatMap[A, B, C, D](
      rab: R[A, B]
  )(fl: A => L[C, D], fr: B => R[C, D]): R[C, D]
...
trait TwinMonad[F[_, _]] extends BiMonad[F, F] with Bifunctor[F] { self =>
  ...
  def foldWith[E, A, X, R](
      fa: F[E, A]
  )(h: E => F[X, R], f: A => F[X, R]): F[X, R]
  def foldWithC[E, A, X, R](fa: F[E, A])(h: E => F[X, R])(
      f: A => F[X, R]
  ): F[X, R] = foldWith(fa)(h, f)
  ...
  def tapBoth[E, A, X, B, Y, C](
      fa: F[E, A]
  )(h: E => F[X, B], f: A => F[Y, C]): F[E, A] =
  ...
  def swap[E, A](fab: F[E, A]): F[A, E] =
    foldWith[E, A, A, E](fab)(e => pure(e), a => raise(a))
  ...
  def leftFlatMap[A, B, C, D](
      lab: F[A, B]
  )(fl: A => F[C, D], fr: B => F[C, D]): F[C, D] = foldWith(lab)(fl, fr)

  def rightFlatMap[A, B, C, D](
      rab: F[A, B]
  )(fl: A => F[C, D], fr: B => F[C, D]): F[C, D] = foldWith(rab)(fl, fr)
...
trait Bind[F[_, _]] extends TwinMonad[F] { self =>
  def foldRec[E, A, X, B](init: Either[E, A])(
      step: Either[E, A] => F[Either[E, X], Either[A, B]]
  ): F[X, B]
  ...
  def lmonad[R]: Monad[F[*, R]] = new Monad[F[*, R]] {
    override def flatMap[A, B](fa: F[A, R])(f: A => F[B, R]): F[B, R] =
      self.handleWith(fa, f)

    override def tailRecM[A, B](a: A)(f: A => F[Either[A, B], R]): F[B, R] =
      self.tailRecHandle(a)(f)
...
trait StackSafeBind[F[_, _]] extends Bind[F] { self =>
  override def foldRec[E, A, X, B](
      init: Either[E, A]
  )(step: Either[E, A] => F[Either[E, X], Either[A, B]]): F[X, B] =
